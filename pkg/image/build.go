...
func traceTime(start time.Time, key string, id string, action string) time.Time {
	elapsed := time.Since(start).Milliseconds()
	console.Infof("[TIMING] %s %s %s %dms\n", key, id, action, elapsed)
	return time.Now()
}
...
func Build(cfg *config.Config, dir, imageName string, progressOutput string, writer io.Writer, imagesToPull []string) (string, error) {
	...
	startTime = traceTime(startTime, imageName, imageName, "start")
